              ...
              <AnimatePresence>
                {errorMsg && (
                  <motion.div
                    ...
                    animate={{ opacity: 1, y: 0, height: 'auto' }}
                    exit={{ opacity: 0, y: -10, height: 0 }}
                    className="flex items-start gap-3 bg-red-50 border border-red-200 rounded-2xl p-4 overflow-hidden"
                  >
                    <AlertCircle className="w-5 h-5 text-red-500 flex-shrink-0 mt-0.5" />
                    <div>
                      <p className="text-sm font-semibold text-red-700">Assignment Failed</p>
                  ...
                  <div className="space-y-2">
                    {pendingDrivers.map(d => (
                      <motion.div
                        ...
                        className={`flex items-center gap-3 p-3 rounded-2xl border cursor-pointer transition-all ${selectedDriver?.id === d.id
                            ? 'bg-emerald-50 border-emerald-300 shadow-sm'
                            : 'bg-gray-50 border-gray-100 hover:border-gray-200'
                          }`}
                      >
                        <div className={`w-10 h-10 rounded-xl flex items-center justify-center text-white text-sm font-bold flex-shrink-0 ${selectedDriver?.id === d.id ? 'gradient-green' : 'bg-gray-300'
                          }`}>
                          {d.avatar}
                        </div>
                  ...
                  >
                    ...
                    <div className="relative">
                      <button
                        ...
                        className={`w-full flex items-center gap-3 px-4 py-3 bg-white border rounded-2xl text-sm font-medium transition-all ${vehicleOpen ? 'border-emerald-400 ring-2 ring-emerald-200' : 'border-gray-200 hover:border-gray-300'
                          }`}
                      >
                          ...
                          >
                            {availableVehicles.length === 0 ? (
                              <div className="px-4 py-4 text-sm text-gray-500 text-center font-medium">
                                No unallocated vehicles available
                              </div>
                            ) : (
                              availableVehicles.map(v => (
                                <button
                                  ...
                                  className={`w-full flex items-center gap-3 px-4 py-3 text-sm hover:bg-gray-50 transition-colors text-left ${selectedVehicle?.id === v.id ? 'bg-emerald-50' : ''
                                    }`}
                                >
                                  <div className={`w-8 h-8 rounded-xl flex items-center justify-center flex-shrink-0 ${v.status === 'running' ? 'bg-emerald-100' :
                                      v.status === 'charging' ? 'bg-blue-100' :
                                        v.status === 'idle' ? 'bg-amber-100' : 'bg-red-100'
                                    }`}>
                                    <Zap className={`w-3.5 h-3.5 ${v.status === 'running' ? 'text-emerald-600' :
